...
# Resize to 160x144, focusing on the center
w, h = img.size
...
text_str = "GAME OVER!"
# Double scale the font for readability
scale = 2
char_w = 5 * scale
char_h = 7 * scale
spacing = 1 * scale
...
# First, draw a black background box for the text to make it readable
box_pad = 2
...
for y in range(0, 144, 8):
    for x in range(0, 160, 8):
        ...
        tiles.append(tuple(map(tuple, tile)))
...
# If we have too many tiles, we need to reduce detail.
if len(unique_tiles) > 256:
    print("WARNING: More than 256 unique tiles! Attempting to force tile matching...")
    reduced_unique = unique_tiles[:256]
...
tile_map = []
for t in tiles:
    tile_map.append(unique_tiles.index(t))
